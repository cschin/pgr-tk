...
pub fn naive_dbg_consensus(
    ...
    let get_best_path = |kmers: Vec<usize>| -> Vec<u8> {
        ...
        kmers.into_iter().for_each(|m| {
            ...
            let ms = *kmer_count
                .get(&m)
                .expect("invariant: kmer_count populated for all graph nodes");
            in_edges.into_iter().for_each(|(v, _w, _)| {
                if bn.is_none() {
                    bs = *node_score
                        .get(&v)
                        .expect("invariant: node_score set in topological order");
                    bn = Some(v);
                } else {
                    let s = *node_score
                        .get(&v)
        ...
        (0..tgt_seq.len() - kmer_size + 1).for_each(|p| {
            if p != 0 {
                ...
                let idx0 = *kmer_idx
                    .get(&kmer0)
                    .expect("invariant: kmer indexed during build");
        ...
        // println!("{:?}", last_kmer);
        let last_tgt_idx = *kmer_idx
            .get(&last_kmer.to_vec())
            .expect("invariant: last kmer indexed during build");
        ...
        loop {
            if let Some(Some(p_idx)) = tgt_rev_path.get(&cur_idx) {
                if *kmer_count
                    .get(p_idx)
                    .expect("invariant: kmer_count populated for all path nodes")
                    >= min_cov
                {
                    cur_idx = *p_idx;
                    rev_path.push(cur_idx);
...
pub fn shmmr_dbg_consensus(
    ...
    while let Some((node, p_node, is_leaf, rank, branch_id, branch_rank)) = wdfs_walker.next(&g) {
        let node_count = *score
            .get(&node)
            .expect("invariant: score set for all graph nodes");
    ...
    for (node, _p_node, node_count, is_leaf, _rank, _branch_id, _branch_rank) in out {
        if out_seq.is_empty() {
            let seq = frg_seqs
                .get(&node)
                .expect("invariant: frg_seqs populated for all graph nodes")
                .clone();
            for _ in 0..seq.len() {
...
pub fn guided_shmmr_dbg_consensus(
    ...
    let w = *score
        .get(&start)
        .expect("invariant: start node has a score");
        ...
        if visited.insert(node.1) {
            ...
            for succ in g.neighbors_directed(node.1, Outgoing) {
                ...
                if !visited.contains(&succ) {
                    ...
                    let s = *score
                        .get(&succ)
                        .expect("invariant: score set for all successor nodes");
                    if guide_nodes.contains_key(&succ) {
                        // choose the closest one for repetitive cases
                        if let Some(pos) = current_node_position {
                            let pos2 = *guide_nodes
                                .get(&succ)
                                .expect("invariant: succ was just found to be in guide_nodes");
                            if pos2 > *pos {
            ...
            if let Some(node) = next_guide_node {
                ...
                succ_list_f.sort();
                next_node = succ_list_f
                    .pop()
                    .expect("invariant: succ_list_f is non-empty after is_empty() check");
            } else {
                break;
            }
            out.push((
                ...
                *score
                    .get(&node.1)
                    .expect("invariant: score set for all visited nodes"),
            ));
        }
    }
    ...
    for (node, node_count) in out {
        ...
        */
        if last_in_guide_nodes.is_some()
            && node == last_in_guide_nodes.expect("invariant: just checked is_some()")
        {
            break;
        }
    }
...
pub fn shmmr_sparse_aln_consensus(
    ...
    let out = shmmr_sparse_aln_consensus_with_sdb(vec![0], &sdb, min_cov)
        .expect("algorithm invariant: consensus with valid SDB should succeed");
...
pub fn shmmr_sparse_aln_consensus_with_sdb(
    ...
    fn shmmr_sparse_aln_consensus_with_sdb_one(
        ...
        keys.into_iter().for_each(|k| {
            let m = hit_map
                .get(&k)
                .expect("invariant: k came from hit_map.keys()");
        ...
        reliable_regions.into_iter().for_each(|(r, c)| {
            if p_region.is_none() {
                ...
                if r.0
                    == p_region
                        .expect("invariant: p_region is Some after else branch")
                        .0
                         .1
                {
                    seq.extend(seq0[r.0 as usize..r.1 as usize].to_vec());
                    ...
                    let p_hit = hit_map
                        .get(
                            &p_region
                                .expect("invariant: p_region is Some in else branch")
                                .0,
                        )
                        .expect("invariant: p_region key in hit_map");
                    let c_hit = hit_map.get(&r).expect(
                        "invariant: r came from reliable_regions which came from hit_map keys",
                    );
                    let p_hit = p_hit
                        .iter()
                        .copied()
                    ...
                    for (sid, v) in p_hit {
                        ...
                        if c_hit.contains_key(&sid) {
                            let w = *c_hit
                                .get(&sid)
                                .expect("invariant: sid found in c_hit after contains_key check");
