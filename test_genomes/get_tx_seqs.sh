...
../target/release/pgr align liftover-gtf \
    ...
    --full-coverage 0.9 \
    --ref-fa   "$REF_FA" \
    --query-fa hg002v1.1.mat_MT.PanSN.fa.gz
...
../target/release/pgr align liftover-gtf \
    ...
    --full-coverage 0.9 \
    --ref-fa   "$REF_FA" \
    --query-fa hg002v1.1.pat.PanSN.fa.gz

# ---------------------------------------------------------------------------
# HTML report
# ---------------------------------------------------------------------------
echo "=== Generating HTML report: $REPORT ==="
python3 - hg002_hap0_liftover.db hg002_hap1_liftover.db "$REPORT" <<'PYEOF'
import sys, sqlite3, json, html

db0_path, db1_path, out_path = sys.argv[1], sys.argv[2], sys.argv[3]
...
total_genes = query(db0_path, "SELECT COUNT(DISTINCT gene_name) FROM transcripts WHERE gene_name!=''")[0][0]
...
# Plot 1: spliced exon length ref vs contig  (units: kb)
...
# Plot 2: genomic span ref vs contig  (units: kb)
...
def nm_funnel_html(f, label):
    ...
    s = f"<h3>{html.escape(label)}</h3><table class='data'><thead><tr><th>Category</th><th>Count</th><th>% of total NM</th></tr></thead><tbody>"
    for name, n, color in rows:
        s += f"<tr><td>{name}</td><td class='num'>{n:,}</td><td>{bar(n/f['total']*100, color)}</td></tr>"
    s += "</tbody></table>"
    return s
...
def chr_table(rows, label):
    ...
    s += ("<table class='data'><thead><tr>"
          ...
          "<th>Partial</th><th>No hit</th><th>HQ%</th></tr></thead><tbody>")
...
# Coverage histogram data for chart.js
cov_labels = [str(int(b)) for b, _ in cov_dist]
cov_values = [n for _, n in cov_dist]
cov_labels_js = json.dumps(cov_labels)
cov_values_js = json.dumps(cov_values)
...
<div class="tabpanel active" id="tab-summary">
  <p class="note">
    Reference: GRCh38 · Annotation: hg38.ncbiRefSeq.gtf.gz ·
    Hap0 = maternal (mat_MT) · Hap1 = paternal (pat) ·
    HQ threshold: ≥ 90% exon coverage on a single contig ·
    Coordinate mapping uses chain-aware gap projection
  </p>
  <div class="summary-grid">
    <div class="card"><div class="val">{total_genes:,}</div><div class="lbl">total annotated gene names</div></div>
    <div class="card"><div class="val">{hq0:,}</div><div class="lbl">HQ genes — hap0</div></div>
    <div class="card"><div class="val">{hq1:,}</div><div class="lbl">HQ genes — hap1</div></div>
    <div class="card"><div class="val">{f0['hq']:,}</div><div class="lbl">HQ NM curated genes — hap0</div></div>
    <div class="card"><div class="val">{f1['hq']:,}</div><div class="lbl">HQ NM curated genes — hap1</div></div>
    <div class="card"><div class="val">{f0['no_hit']:,}</div><div class="lbl">NM genes — no hit — hap0</div></div>
  </div>
</div>
...
<div class="tabpanel" id="tab-nmgenes">
  ...
  <div class="col2">
    {tx_type_table(tx_types0, "Hap0 — HQ gene count by transcript type")}
    {tx_type_table(tx_types1, "Hap1 — HQ gene count by transcript type")}
  </div>
  <h2>NM Gene Coverage Distribution — Hap0</h2>
...
<div class="tabpanel" id="tab-scatter">
  ...
  <p class="note">
    Genomic span on the contig (contig_end − contig_start) vs the reference gene body span
    (ref_end − ref_start, including all introns). Deviations from the diagonal indicate
    structural differences (insertions, deletions, or local rearrangements).
...
<script>
// Charts are lazy-initialised the first time each tab is shown
const _chartsDone = {{}};
...
// Init summary tab charts on load
...
new Chart(document.getElementById('covChart'), {{
  ...
  options: {{
    ...
    scales: {{
      ...
      y: {{ title: {{ display: true, text: 'NM genes' }},
            beginAtZero: true }}
    }}
  }}
}});
...
echo ""
echo "Done."
echo "  hg002_hap0_liftover.db / .gtf / .hq.gtf / .anomaly.tsv / .gene.anomaly.tsv"
echo "  hg002_hap0_liftover.ref_tx.fa / .contig_tx.fa / .hq_contig_tx.fa"
echo "  hg002_hap1_liftover.db / .gtf / .hq.gtf / .anomaly.tsv / .gene.anomaly.tsv"
echo "  hg002_hap1_liftover.ref_tx.fa / .contig_tx.fa / .hq_contig_tx.fa"
echo "  $REPORT"
