...
A \emph{shimmer} is a minimizer-style sparse $k$-mer selected within a sliding
window of size $w$ by a reduction factor $r$.
Formally, for a sequence $S$ of length $L$, shimmers are the subset of
$(w,k,r)$-minimizers retained after applying a modular reduction:
only positions $p$ where $H(k\text{-mer at }p) \bmod r = 0$ are kept,
where $H$ is a deterministic hash function.

\begin{definition}[Shimmer]
  Given parameters $(w, k, r)$, a position $p$ in sequence $S$ defines
  a shimmer if:
  \begin{enumerate}[noitemsep]
    \item The $k$-mer at position $p$ is the lexicographic minimum within
          the window $[p - w/2,\; p + w/2]$, and
    \item $H(S[p \ldots p+k-1]) \bmod r = 0$.
  \end{enumerate}
\end{definition}

This two-level selection yields an index that is both sparse (fast to query)
and sensitive (each shimmer represents a conserved sequence anchor).
...
\ForAll{sequence $T \in \mathcal{T}$}
  \State Extract all shimmers from $T$ with parameters $(w,k,r)$
  \ForAll{consecutive shimmer pair $(s_i, s_{i+1})$ with span $\geq \text{min\_span}$}
