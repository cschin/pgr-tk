...
tsv_path  = args.timelog if args.timelog else os.path.join(base_dir, "run_all_timings.tsv")
...
def _f(s):
    """Parse a timing field to float; return 0.0 for N/A or non-numeric values."""
    try:
        return float(s)
...
for r in rows:
    ...
    wall_s = f"{wall_f:.1f}s"    if wall_f  or wall  == "0" else wall
