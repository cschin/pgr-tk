...
#
# Files downloaded to downloads/ (~2.5 GB total):
...
CSV_CACHE="$DL/hprc_r2_index.csv"
...
dl "$GRCh38_URL"    "$DL/$GRCh38_FILE"
dl "$HG002_mat_URL" "$DL/$HG002_mat_FILE"
dl "$HG002_pat_URL" "$DL/$HG002_pat_FILE"
...
dl "$GTF_URL"         "$DL/hg38.ncbiRefSeq.gtf.gz"
dl "$CLINVAR_URL"     "$DL/clinvar.vcf.gz"
dl "$CLINVAR_TBI_URL" "$DL/clinvar.vcf.gz.tbi"

# ---------------------------------------------------------------------------
# 5. Write a manifest for downstream scripts to source
# ---------------------------------------------------------------------------
if [[ -f ".manifest.sh" ]]; then
    echo "[SKIP] .manifest.sh already exists"
else
    cat > .manifest.sh <<EOF
# Auto-generated by 00_download.sh — do not edit by hand
REF_FA="$DL/${GRCh38_FILE}"
HAP0_FA="$DL/${HG002_mat_FILE}"
HAP1_FA="$DL/${HG002_pat_FILE}"
GTF="$DL/hg38.ncbiRefSeq.gtf.gz"
CLINVAR="$DL/clinvar.vcf.gz"
EOF
...
echo
ls -lh "$DL/$GRCh38_FILE" "$DL/$HG002_mat_FILE" "$DL/$HG002_pat_FILE" \
        "$DL/hg38.ncbiRefSeq.gtf.gz" "$DL/clinvar.vcf.gz"
